        ...
        try:
            ...
            try:
                if import_job.status == ImportStatus.COMPLETED.value and import_job.success_count > 0:
                    logger.info(f"[BG] Job {job_id}: Starting automatic ID writeback")
                    ...
                    if writeback_result.get('success'):
                        logger.info(
                            f"[BG] Job {job_id}: Auto-writeback succeeded - "
                            f"{writeback_result.get('mappings_count', 0)} mappings stored"
                        )
                    else:
                        logger.warning(
                            f"[BG] Job {job_id}: Auto-writeback failed - "
                            f"{writeback_result.get('error', 'Unknown error')}"
                        )
                else:
                    logger.info(
                        f"[BG] Job {job_id}: Skipping auto-writeback "
